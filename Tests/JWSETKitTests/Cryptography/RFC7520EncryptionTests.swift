...
struct RFC7520EncryptionTests {
    ...
    func testEncrypt_RSA_v1_5() throws {
        ...
        #expect(jwe.header.protected.encoded == header)
        #expect(jwe.sealed.nonce == "bbd5sTkYwhAIqfHsx8DayA".decoded)
        #expect(jwe.sealed.ciphertext == """
        ...
        i7lzA6BP430m
        """.decoded)
        #expect(jwe.sealed.tag == "kvKuFBXHe5mQr4lqgobAUg".decoded)
        #expect(try jwe.decrypt(using: RFC7520ExampleKeys.rsa1_5EncPrivateKey.key) == plainText.data)
    }
    ...
    func testEncrypt_RSA_OAEP() throws {
        ...
        #expect(jwe.header.protected.encoded == header)
        #expect(jwe.sealed.nonce == "-nBoKLH0YkLZPSI9".decoded)
        #expect(jwe.sealed.ciphertext == """
        ...
        """.decoded)
        #expect(jwe.sealed.tag == "UCGiqJxhBI3IFVdPalHHvA".decoded)
    ...
    func testEncrypt_PBES2() throws {
        ...
        #expect(jwe.header.protected.encoded == header)
        #expect(jwe.encryptedKey == "d3qNhUWfqheyPp4H8sjOWsDYajoej4c5Je6rlUtFPWdgtURtmeDV1g".decoded)
        #expect(jwe.sealed.nonce == "VBiCzVHNoLiR3F4V82uoTQ".decoded)
        #expect(jwe.sealed.ciphertext == """
        ...
        3kobXZ77ulMwDs4p
        """.decoded)
        #expect(jwe.sealed.tag == "0HlwodAhOCILG5SQ2LQ9dg".decoded)
        #expect(try jwe.decrypt(using: pbkdf2Key) == plainText.data)
    }
    ...
    func testEncrypt_ECDH_ES_AESKW() throws {
        ...
        #expect(jwe.header.protected.encoded == header)
        #expect(jwe.encryptedKey == "0DJjBXri_kBcC46IkU5_Jk9BqaQeHdv2".decoded)
        #expect(jwe.sealed.nonce == "mH-G2zVqgztUtnW_".decoded)
        #expect(jwe.sealed.ciphertext == """
        ...
        """.decoded)
        #expect(jwe.sealed.tag == "WuGzxmcreYjpHGJoa17EBg".decoded)
        #expect(try jwe.decrypt(using: RFC7520ExampleKeys.ecdhPrivateKey.key) == plainText.data)
    }
    ...
    func testEncrypt_ECDH_ES_AESCBC() throws {
        ...
        #expect(jwe.header.protected.encoded == header)
        #expect(jwe.encryptedKey == nil)
        #expect(jwe.sealed.nonce == "yc9N8v5sYyv3iGQT926IUg".decoded)
        #expect(jwe.sealed.ciphertext == """
        ...
        95_JGG2m9Csg
        """.decoded)
        #expect(jwe.sealed.tag == "WCCkNa-x4BeB9hIDIfFuhg".decoded)
        #expect(try jwe.decrypt(using: kek.key) == plainText.data)
    }
    ...
    func testEncrypt_Direct_AESGCM() throws {
        ...
        #expect(jwe.header.protected.encoded == header)
        #expect(jwe.encryptedKey == nil)
        #expect(jwe.sealed.nonce == "refa467QzzKx6QAB".decoded)
        #expect(jwe.sealed.ciphertext == """
        ...
        """.decoded)
        #expect(jwe.sealed.tag == "vbb32Xvllea2OtmHAdccRQ".decoded)
        #expect(try jwe.decrypt(using: key) == plainText.data)
    }
    ...
    func testEncrypt_AESGCMKW() throws {
        ...
        #expect(jwe.header.protected.encoded == header)
        #expect(jwe.encryptedKey == "lJf3HbOApxMEBkCMOoTnnABxs_CvTWUmZQ2ElLvYNok".decoded)
        #expect(jwe.sealed.nonce == "gz6NjyEFNm_vm8Gj6FwoFQ".decoded)
        #expect(jwe.sealed.ciphertext == """
        ...
        1B-gxpNk3xWU
        """.decoded)
        #expect(jwe.sealed.tag == "DKW7jrb4WaRSNfbXVPlT5g".decoded)
        #expect(try jwe.decrypt(using: RFC7520ExampleKeys.keyWrapGCMSymmetricKey.key) == plainText.data)
    }
    ...
    func testEncrypt_AESKW() throws {
        ...
        #expect(jwe.header.protected.encoded == header)
        #expect(jwe.encryptedKey == "CBI6oDw8MydIx1IBntf_lQcw2MmJKIQx".decoded)
        #expect(jwe.sealed.nonce == "Qx0pmsDa8KnJc9Jo".decoded)
        #expect(jwe.sealed.ciphertext == """
        ...
        """.decoded)
        #expect(jwe.sealed.tag == "ER7MWJZ1FBI_NKvn7Zb1Lw".decoded)
    ...
    func testEncrypt_AESKW_Deflate() throws {
        ...
        #expect(jwe.header.protected.encoded == header)
        #expect(jwe.encryptedKey == "5vUT2WOtQxKWcekM_IzVQwkGgzlFDwPi".decoded)
        #expect(jwe.sealed.nonce == "p9pUq6XHY0jfEZIl".decoded)
        #expect(jwe.sealed.ciphertext == """
        ...
        """.decoded)
        #expect(jwe.sealed.tag == "VILuUwuIxaLVmh5X-T7kmA".decoded)
        #expect(jwe.sealed.ciphertext.count < plainText.data.count)
        #expect(try jwe.decrypt(using: RFC7520ExampleKeys.keyWrapSymmetricKey.key) == plainText.data)
    }
    ...
    func testEncrypt_AES_KW_AAD() throws {
        ...
        #expect(jwe.header.protected.encoded == header)
        #expect(jwe.encryptedKey == "4YiiQ_ZzH76TaIkJmYfRFgOV9MIpnx4X".decoded)
        #expect(jwe.sealed.nonce == "veCx9ece2orS7c_N".decoded)
        #expect(jwe.sealed.ciphertext == """
        ...
        """.decoded)
        #expect(jwe.sealed.tag == "vOaH_Rajnpy_3hOtqvZHRA".decoded)
        #expect(try jwe.decrypt(using: RFC7520ExampleKeys.keyWrapSymmetricKey.key) == plainText.data)
    }
    ...
    func testEncrypt_AES_KW_PartialUnprotectedHeader() throws {
        ...
        #expect(jwe.header.protected.encoded == header)
        #expect(jwe.encryptedKey == "jJIcM9J-hbx3wnqhf5FlkEYos0sHsF0H".decoded)
        #expect(jwe.sealed.nonce == "WgEJsDS9bkoXQ3nR".decoded)
        #expect(jwe.sealed.ciphertext == """
        ...
        """.decoded)
        #expect(jwe.sealed.tag == "fNYLqpUe84KD45lvDiaBAQ".decoded)
        #expect(try jwe.decrypt(using: RFC7520ExampleKeys.keyWrapSymmetricKey.key) == plainText.data)
    }
    ...
    func testEncrypt_AES_KW_UnprotectedHeader() throws {
        ...
        #expect(jwe.header.protected.encoded == header)
        #expect(jwe.encryptedKey == "244YHfO_W7RMpQW81UjQrZcq5LSyqiPv".decoded)
        #expect(jwe.sealed.nonce == "YihBoVOGsR1l7jCD".decoded)
        #expect(jwe.sealed.ciphertext == """
        ...
        """.decoded)
        #expect(jwe.sealed.tag == "e2m0Vm7JvjK2VpCKXS-kyg".decoded)
        #expect(try jwe.decrypt(using: RFC7520ExampleKeys.keyWrapSymmetricKey.key) == plainText.data)
    }
}
