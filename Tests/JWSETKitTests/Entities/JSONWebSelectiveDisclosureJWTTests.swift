...
struct JSONWebSelectiveDisclosureJWTTests {
    ...
    @Test("Verify key binding never skips signature check without a resolvable holder key")
    func verifyKeyBindingNoConfirmationNoHolderKey() throws {
        let issuerKey = P256.Signing.PrivateKey()
        // Holder key is NOT attested by any `cnf` claim — an attacker self-signing a KB-JWT.
        let attackerKey = P256.Signing.PrivateKey()
        
        let claims = try JSONWebTokenClaims {
            $0.subject = "user123"
            // Note: no `confirmation` (cnf) claim.
        }
        let sdJWT = try JSONWebSelectiveDisclosureToken(claims: claims, using: issuerKey)
        let presentation = try sdJWT.withKeyBinding(
            using: attackerKey,
            algorithm: .ecdsaSignatureP256SHA256,
            nonce: "test-nonce",
